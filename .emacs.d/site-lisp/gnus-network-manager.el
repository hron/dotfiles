;;; gnus-network-manager.el --- Gnus network manager integration.

;; Copyright (C) 2010  Øyvind Stegard
...
;; Somewhat experimental integration between Gnus agent and NetworkManager over
;; D-Bus. Kill IMAP connections and unplug Gnus agent when network goes down,
;; plug agent when network comes up.

;; I find this useful when running Emacs+Gnus on my laptop, where network
;; conditions often change, especially on suspend/resume. It was annoying that
;; Gnus often hung as a result of these events, usually caused by hanging
;; network sub-processes (openssl, gnutls-cli).

;; Requires Emacs23 with D-Bus bindings and NetworkManager running. Probably
;; only useful if you’re running Emacs on a modern GNU/Linux distribution of
;; some sort.
...
;; If you want to start up Gnus in offline or online state depending on the
;; current network status, you can add a custom Gnus startup function in
;; ~/.emacs, something like this:
...
(provide 'gnus-network-manager)
;;; gnus-network-manager.el ends here
